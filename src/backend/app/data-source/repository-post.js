...
const {
  post,
  datasource,
  postIndexHistory: PostIndexHistory,
} = db.sequelize.models;
  ...
  try {
    const data = await post.findAll({
      ...
      attributes: [
        "datasource",
        "index_status",
        [Sequelize.fn("COUNT", "index_status"), "count"],
      ],
      raw: true,
    ...
    const result = {};
    data.forEach((post) => {
      if (!result[post.datasource])
        result[post.datasource] = { ...responseStructure };
