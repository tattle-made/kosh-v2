...
async function readLines(basePath) {
    ...
    for await (const line of rl) {
        ...
        const imageFileStream = fs.createReadStream(path.join(basePath, imageFileName));
...
const insertPost = async () => {
    ...
    const folders = fs.readdirSync(reisDatasetFolder).filter(item => !(/(^|\/)\.[^\/\.]/g).test(item))
    for (const country of folders) {
        const imageFolders = fs.readdirSync(`./${reisDatasetFolder}/${country}`).filter(item => !(/(^|\/)\.[^\/\.]|.txt/g).test(item))
        for (const type of imageFolders) {
            const posts = await readLines(`./${reisDatasetFolder}/${country}/${type}`, { encoding: 'utf-8'})
