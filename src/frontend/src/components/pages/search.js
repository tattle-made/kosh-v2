...
import { post as postToIndexApi, postFormData as postFormDataToIndexApi } from "../../service/indexer-api";
...
const SearchPost = () => {
    ...
    const onDrop = (acceptedFiles) => {
        ...
        postFormDataToIndexApi('/search', formData).then((response) => {
    ...
    const getPosts = (searchResult) => {
        const postIds = searchResult.map((post) => post.source_id)
        postWithToken("/posts", {postIds}).then((response) => setPosts(response.data))
    ...
    const searchWithText = () => {
        ...
        postToIndexApi('/search', JSON.stringify(payload)).then((response) => {
            if (!response.data.matches || !response.data.matches.length) return
            getPosts(response.data.matches)
        }).catch((res) => console.log(res))
    }
    ...
    return (
        <>
            {showFilter ? <SearchSettings selectedDatasource={selectedDatasource}
