...
const PostsIndex = ({ location }) => {
  ...
  const postIndexStatus = () => {
    get(`/index/datasource/${datasourceId}/post`)
      .then((response) => {
  ...
  const filterPosts = () => {
    ...
    return setFilteredPosts(
      posts.filter((post) => filter.includes(post.index_status))
    );
  };
  ...
  const retryIndex = () => {
    const postIds = filteredPosts
      ...
      .map((post) => post.id);
    postWithToken(`/index/datasource/${datasourceId}/post`, { postIds })
      .then(() => {
        postIndexStatus();
      })
      .catch((error) => {
  ...
  const blacklistIndex = () => {
    const postIds = filteredPosts
      .filter((post) => post.checked)
      .map((post) => post.id);
    patch(`/index/datasource/${datasourceId}/post/blacklist`, { postIds })
      .then(() => {
        postIndexStatus();
      })
      .catch((error) => {
    ...
    return (
      ...
      >
        ...
        <Box pad={"small"} overflow={"hidden"} width={"medium"}>
          <Text wordBreak={"break-all"} size={"xsmall"}>
            {post.id}
          </Text>
          {post.index_status ? (
            <Box
              ...
              margin={{ top: "xsmall" }}
            >
              <Text size={"xsmall"}>{post.index_status}</Text>
            </Box>
          ) : null}
        </Box>
      </Box>
      ...
      {!posts.length ? (
        ...
        <>
          ...
          <Box margin="medium" direction="row" gap="small">
            ...
            <CheckBox
              checked={failedStatus}
              label={"Failed"}
              onChange={() => setFailedStatus(!failedStatus)}
            />
            <CheckBox
              checked={blacklistedStatus}
              label={"Blacklisted"}
